...
export default function AdminReportsPage() {
  ...
  const handleExportPDF = () => {
    ...
    doc.text(
      "Report Type: Comprehensive System Overview & Integrity Audit",
      14,
      37,
    );
    ...
    const deptBody = departmentStats.map((dept) => [
      dept.deptAbbreviation,
      dept.studentCount,
    ]);
    ...
    if (actionItems.unassignedStudents.length > 0) {
      ...
      doc.text(
        `Warning: ${actionItems.unassignedStudents.length} Student(s) without a Group`,
        14,
        currentY,
      );
      autoTable(doc, {
    ...
    if (actionItems.unassignedGroups.length > 0) {
      doc.setTextColor(220, 38, 38);
      doc.text(
        `Warning: ${actionItems.unassignedGroups.length} Group(s) without a Project`,
        14,
        currentY,
      );
      autoTable(doc, {
        ...
        body: actionItems.unassignedGroups.map((g) => [
          `GRP-${g.groupId}`,
          g.groupLeader,
        ]),
        theme: "plain",
    ...
    if (actionItems.emptyIndustries.length > 0) {
      ...
      doc.text(
        `Notice: ${actionItems.emptyIndustries.length} Registered Industries without Externals`,
        14,
        currentY,
      );
      autoTable(doc, {
        ...
        body: actionItems.emptyIndustries.map((i) => [
          i.industryId,
          i.industryName,
        ]),
        theme: "plain",
      ...
      doc.setTextColor(22, 163, 74);
      doc.text(
        "Success: All registered industries have active external supervisors.",
        14,
        currentY,
      );
    }
      ...
      <div className="p-6 max-w-7xl mx-auto w-full flex flex-col gap-6">
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            />
            <ReportCard
              title="Registered Industries"
              value={summary?.totalIndustries || 0}
              icon={<Briefcase className="w-5 h-5 text-emerald-600" />}
              bgClass="bg-emerald-50"
            />
            <ReportCard
              title="External Supervisors"
              value={summary?.totalExternals || 0}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden flex flex-col">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <UserX
                    className={`w-5 h-5 ${actionItems.unassignedStudents.length > 0 ? "text-red-500" : "text-green-500"}`}
                  />
            ...
            <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden flex flex-col">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <FolderX
                    className={`w-5 h-5 ${actionItems.unassignedGroups.length > 0 ? "text-red-500" : "text-green-500"}`}
                  />
                  <h3 className="font-semibold text-slate-800">
                    Unassigned Groups
                  </h3>
            ...
            <div className="lg:col-span-2 border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden flex flex-col">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Building
                    className={`w-5 h-5 ${actionItems.emptyIndustries.length > 0 ? "text-amber-500" : "text-green-500"}`}
                  />
                  <h3 className="font-semibold text-slate-800">
                    Industries Pending Externals
                  </h3>
                </div>
                <span className="text-xs font-bold px-2 py-1 rounded-full bg-slate-200 text-slate-700">
                  ...
                  <table className="w-full text-left text-sm">
                    <tbody className="divide-y divide-slate-100">
                      {actionItems.emptyIndustries.map((ind) => (
                        ...
                        >
                          <td className="px-4 py-3 font-medium text-slate-900 w-24">
                            ID: {ind.industryId}
                          </td>
                          <td className="px-4 py-3 text-slate-600">
                            {ind.industryName}
