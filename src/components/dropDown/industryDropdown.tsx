      ...
      >
        {selectedIndustry && (
          <span className="flex items-center gap-1.5 bg-slate-100 text-slate-700 text-sm px-2 py-0.5 rounded max-w-[calc(100%-2rem)]">
            <span className="truncate">{selectedIndustry.industryName}</span>
            <X
        ...
        <input
          ...
          placeholder={!selectedIndustry ? placeholder : ""}
        ...
        <div className="absolute top-full mt-1 w-full bg-white border border-slate-200 rounded-md shadow-lg z-50 max-h-60 overflow-y-auto py-1">
          {availableOptions.length > 0 ? (
            availableOptions.map((opt) => (
              <div
                key={opt.industryId}
                className="px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 cursor-pointer transition-colors truncate"
                onClick={() => {
                  onChange(opt.industryName);
                  setSearchTerm("");
                  setIsOpen(false);
                }}
              >
