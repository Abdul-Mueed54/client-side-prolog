...
export default function GroupDropdown({
  ...
  const availableOptions = options.filter(
    (opt) =>
      opt.groupId !== value &&
      opt.groupId.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const selectedGroup = options.find((opt) => opt.groupId === value);
      ...
      >
        {selectedGroup && (
          <span className="flex items-center gap-1.5 bg-slate-100 text-slate-700 text-sm px-2 py-0.5 rounded max-w-[calc(100%-2rem)]">
            <span className="truncate">{selectedGroup.groupId}</span>
            <X
